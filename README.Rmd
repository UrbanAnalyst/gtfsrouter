...
package function `berlin_gtfs_to_zip()`.
...
stored as a single `.zip` file. Routing is then as simple as the following code:
```{r example}
...
`r round (st[3] * 100) / 100` seconds. (Note that non-ASCII symbols have been
removed from the test data, so "Schönleinstr" has become "Schonleinstr", but the
package will operate with the full VBB feed and proper station matches such as
`from = "Schönlein"`.)
