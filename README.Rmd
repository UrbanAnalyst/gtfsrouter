...
head(x)
```
```{r isochrone, eval = TRUE, echo = FALSE}
gtfs <- extract_gtfs (filename)
gtfs <- gtfs_timetable (gtfs) # A pre-processing step to speed up queries
x <- gtfs_isochrone (gtfs,
                     from = "Schonlein",
...
The function returns a `data.frame` of all stations, with `in_isochrone = TRUE`
for those reachable from the nominated station within the nominated time. The
results of `gtfs_isochrone` can also be plotted as an interactive web map,
simply by `plot(x)`. This requires the packages
