...
That number is equal to the total number of stops in the feed, although in
general the
[`gtfs_traveltimes()`](https://atfutures.github.io/gtfs-router/reference/gtfs_isochrone.html)
will return travel times to only a subset of all potentially reachable
stations, for two main reasons. First, not all stations may actually be
reachable from a given station, and travel times will only be returned for
those that are. Secondly, the 
[`gtfs_traveltimes()`](https://atfutures.github.io/gtfs-router/reference/gtfs_isochrone.html)
