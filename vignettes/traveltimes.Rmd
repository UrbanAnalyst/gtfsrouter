...
calculates travel times from a nominated departure station and time to all
reachable stations. In addition to specifying a departure station, the main
parameter required by the function is a pair of `start_time_limits` specifying
the earliest and latest possible start times. For example, travel times to all
stations for services leaving a nominated station between 12:00 and 13:00 can
be extracted by specifying `start_time_limits = c (12 * 3600, 13 * 3600)`. The
following code uses the internal Berlin data to demonstrate, starting by
reading the GTFS data and constructing a timetable for a specified day, in
order to speed up subsequent routing calculations.
...
The 
[`gtfs_traveltimes()`](https://atfutures.github.io/gtfs-router/reference/gtfs_isochrone.html)
function may then be called with the following code:
...
The total number of stations contained within the sample GTFS data are:
...
The
[`gtfs_traveltimes()`](https://atfutures.github.io/gtfs-router/reference/gtfs_isochrone.html)
function does not generally record travel times to every station within
a system. The number of stations reached is generally controlled by an
additional parameter, `max_traveltime`, which defaults to one hour. The maximum
traveltimes returned from the above call are:
...
trip duration is indeed precisely one hour. The reason the function does not
automatically return information of all reachable stations is that calculation
times increase non-linearly with numbers of stations reached. The following
code estimates calculation times as a function both of `max_traveltime`, and
numbers of stations reached.
...
stations reached, yet increase computation times considerably. Values for
...
And minimal-transfer trips take on average just under 4 minutes longer, while
