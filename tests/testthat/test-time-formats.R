...
test_that ("convert-time", {
    ...
    msg <- paste0 (
        "Day not specified; extracting timetable for ",
        day
    )
    ...
    expect_silent (route1 <- gtfs_route (gt,
        from = from, to = to,
    ...
    start_time <- c (12, 2, 0)
    expect_silent (route5 <- gtfs_route (gt,
        from = from, to = to,
        start_time = start_time,
        quiet = TRUE
    ))
    ...
    expect_silent (route6 <- gtfs_route (gt,
        from = from, to = to,
        start_time = "12:02:00",
        quiet = TRUE
    ))
    ...
    expect_silent (route7 <- gtfs_route (gt,
        from = from, to = to,
        start_time = "12:02",
        quiet = TRUE
    ))
